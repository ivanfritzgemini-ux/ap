...
export default function AsistenciaMensualPage() {
  ...
  const calcularEstadisticas = () => {
    ...
    const estadisticasPorDia = diasDelMes.filter(d => d.esHabil).map(dia => {
      ...
      const porcentaje = totalEstudiantes > 0 ? (presentes / totalEstudiantes) * 100 : 0
    ...
    const estadisticasPorEstudiante = estudiantes.map(estudiante => {
      ...
      const diasPresente = diasHabiles.filter(dia => 
        asistencias[estudiante.id]?.asistencias[dia.dia] === true
      ).length
      const porcentaje = diasHabiles.length > 0 ? (diasPresente / diasHabiles.length) * 100 : 0
      
      return {
        estudianteId: estudiante.id,
        diasPresente,
        totalDias: diasHabiles.length,
                            ...
                            >
                              {dia.esHabil ? (
                                <Checkbox
                                  ...
                                  disabled={!esFechaHabilitada(dia.dia, parseInt(selectedMes))}
